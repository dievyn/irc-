...
void Server::SetNickname(Client client_obj, std::string str)
{
	...
	client_obj.SetNickname(grab_word(&str[5]));
}

void Server::interpret(int fd, std::string command)
{
	if (grab_line(command) == "CAP LS 302")
		send(fd, "CAP * LS :\r\n", 13, 0);
	else if (grab_word(command) == "NICK")
	{
		SetNickname(this->client_list[find_fd(fd)], command);
		// std::string message = ":myserver 001 " + this->name + " :Welcome to the IRC network\r\n";
		// send(fd, message.c_str(), message.size(), 0);
		// message = ":myserver 002 " + this->name + " :Your host is myserver\r\n";
		// send(fd, message.c_str(), message.size(), 0);
		// message = ":myserver 003 " + this->name + " :Created today\r\n";
		// send(fd, message.c_str(), message.size(), 0);
		// message = ":myserver 004 " + this->name + " myserver 0.1 o o\r\n";
		// send(fd, message.c_str(), message.size(), 0);
		// message = ":myserver 376 " + this->name + " :End of MOTD\r\n";
		// send(fd, message.c_str(), message.size(), 0);
		//	ca marche ??? jspppp
	}
	else if (grab_line(command) == "CAP END")
	{
	...
	else if (grab_line(command) == "PING myserver")
	{
		std::cout << "--Server sent PONG\n";
		send(fd, "PONG :myserver\r\n", 16, 0);
	}
	else if (grab_word(command) == "PASS")
	{
		std::cout << "Reading password: ";
		printAscii(grab_word(&command[5]));
		std::cout << "\n";
		if (grab_word(&command[5]) != this->password)
		{
			std::cout << "------- PASSWORD !\n";
